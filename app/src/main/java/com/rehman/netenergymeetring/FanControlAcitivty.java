...
public class FanControlAcitivty extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_fan_control_acitivty);
        ...
        reference.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists())
                {
                    ...
                    value = snapshot.child("FanNosInfo1").getValue(String.class);
                    value2 = snapshot.child("FanNosInfo2").getValue(String.class);
                    ...
                    {
                        Glide.with(FanControlAcitivty.this)
                                .asGif()
    ...
    private void updateLight2Value(String status)
    {
        ...
        reference.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                reference.child("FanNosInfo2").setValue(status);
                if (status.equals("0"))
                {
                    Toast.makeText(FanControlAcitivty.this, "Fan 2 is OFF", Toast.LENGTH_SHORT).show();
                }else if (status.equals("1"))
                {
                    Toast.makeText(FanControlAcitivty.this, "Fan 2 is ON", Toast.LENGTH_SHORT).show();
                }
            }
    ...
    private void updateLight1Value(String status)
    {
        ...
        reference.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                reference.child("FanNosInfo1").setValue(status);

                if (status.equals("0"))
                {
                    Toast.makeText(FanControlAcitivty.this, "Fan 1 is OFF", Toast.LENGTH_SHORT).show();
                }else if (status.equals("1"))
                {
                    Toast.makeText(FanControlAcitivty.this, "Fan 1 is On", Toast.LENGTH_SHORT).show();
                }
